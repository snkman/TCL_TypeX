...
TCL_Groups_F = [
	...
	if (count (TCL_Custom select 0) > 0) then
	{
		...
		private _sort = (TCL_Initialize select 0);
	...
	_array = _array - (TCL_Initialize select 0);
	...
	(TCL_Initialize select 0) append _array;
	...
	// player sideChat format ["TCL_Groups_F > Conditions > %1 > %2", _group, (count units _group) ];
