...
TCL_EH_Fired_F = [
	...
	if (TCL_Server) then
	{
		...
		{
			if (TCL_Feature select 18) then
			{
				_this call (TCL_EH_Fired_F select 1);
			};
			
			if (TCL_Feature select 23) then
			{
	...
	if (count (TCL_Advancing select 0) < 5) then
	{
		...
		if (_distance > 0) then
		{
			...
			[_unit, _array, _distance] call (TCL_Feature_F select 1);
		};
	};
	...
	if ( (time - _time) < (TCL_Feature select 24) ) then
	{
		(TCL_Suppressed select 1) pushBack _unit;
		
		if ( { (_x == _unit) } count (TCL_Suppressed select 1) > (TCL_Feature select 25) ) then
		{
