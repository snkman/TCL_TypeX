...
TCL_Reinforcement_F = [
	...
	if ( [_enemy, _group] call (TCL_Rating_F select 0) ) then
	{
		...
		private _condition = [_group] call (TCL_Radio_F select 0);
		
		private ["_distance","_units","_radio"];
		...
		if (_distance < _condition) then
		{
			...
			if ( (_units findIf { (alive _x) } > -1) && { (side _group getFriend side _x > 0.6) } ) then
			{
				...
				{
					_radio = [_group, _x] call (TCL_Radio_F select 1);
