...
TCL_Unassign_F = [
	...
	while { (_group in (TCL_Reinforcement select 0) ) } do
	{
		_waypoint = (_group getVariable "TCL_Waypoint");
		
		if ( (units _enemy findIf { ( (alive _x) && { (_group knowsAbout vehicle _x > 0) } ) } > -1) || (units _group findIf { ( (alive _x) && { (_x distance2D _waypoint < _distance) } ) } > -1) ) exitWith
		{
	...
	if (True) then
	{
		...
		if (_vehicle isKindOf "Helicopter") exitWith
		{
			...
			{
				if (_units findIf { (assignedVehicleRole _x select 0 == "Cargo") } > -1) then
				{
	...
	if (_group knowsAbout vehicle _enemy > 0) then
	{
		...
		{_x doWatch (getPos _enemy) } count _units;
