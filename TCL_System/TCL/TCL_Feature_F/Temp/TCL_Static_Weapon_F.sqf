...
TCL_Static_Weapon_F = [
	...
	// while { (True) } do
	{
		...
		{
			// player sideChat format ["TCL_Static_Weapon_F > Ammo > %1", _vehicle];
		...
		_muzzel = weaponState [_vehicle, [0] ] select 1;
		
		_rounds = (_vehicle ammo _muzzel);
		
		// _rounds = (_vehicle ammo currentMuzzle _unit);
		
		// hint str _rounds;
		
		// hint str (allTurrets _vehicle);
		
		// if ( (_unit distance _leader > 100) || (_unit distance _waypoint > 500) ) then
