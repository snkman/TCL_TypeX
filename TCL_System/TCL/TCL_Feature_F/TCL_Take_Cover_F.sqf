...
TCL_Take_Cover_F = [
	...
	_array = ( (TCL_Take_Cover select 0) + (TCL_House_Search select 0) + (TCL_Static_Weapon select 0) );
	...
	if (alive _enemy) then
	{
		...
		if (isNil "_array") then
		{
			...
			player sideChat format ["TCL_Take_Cover_F > Error > %1 > %2 > %3 > %4", _unit, (side _unit), _group, (typeOf _unit) ];
		};
	...
	{
		_marker = [ [_unit, time], (getPos _object), "Icon", [1,1], "mil_triangle", "colorBlue"] call TCL_Create_Marker_F;
	...
	// player sideChat format ["TCL_Take_Cover_F > BoundingBox > %1 > %2", _unit, (boundingBox _object select 1) ];
