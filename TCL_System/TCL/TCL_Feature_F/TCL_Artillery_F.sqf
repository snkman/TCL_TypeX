...
TCL_Artillery_F = [
	...
	if (TCL_Feature select 9) then
	{
		if (floor (random 100) < (TCL_Feature select 10) ) then
		{
			...
			if (_condition select 0) then
			{
				...
				_distance = [_x] call (TCL_Radio_F select 0);
				
				_bool = [_vehicle, _position] call (TCL_Artillery_F select 2);
				
				if ( (_bool) && { (_leader distance _vehicle < _distance) } ) exitWith
				{
	...
	if (alive _logic) then
	{
		...
		if (_time > 0) then
		{
			// player sideChat format ["TCL_Artillery_F > Artillery > %1", _units];
			...
			if (alive _logic) then
			{
				...
				// player sideChat format ["TCL_Artillery_F > Artillery > %1", _time];
