...
TCL_Advancing_F = [
	...
	if ( ("Pistol" call _type) || ("Rifle" call _type) ) then
	{
		...
		if (_weapon isEqualTo _muzzle) then
		{
			...
			_return = (TCL_Feature select 20);
			
			if (_suppressor isEqualTo "") then
			{
				_return = (TCL_Feature select 22);
			};
		}
		else
		{
	...
	{
		if ( (floor (random 100) < 50) && { (_distance > 100) } && { (_group knowsAbout vehicle _object == 0) } ) then
		{
			private _leader = (leader _group);
			...
			// {if ( (floor (random 100) < 50) || (_x == _leader) ) then {_x doWatch (getPos _object) } } count _units;
